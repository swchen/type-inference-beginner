...
let rec applySubstToType (subst: Map<string, Type>, t: Type) =
    ...
    | FunctionType (param, body) ->
        FunctionType(
            param = applySubstToType (subst, param),
    ...
    | ForAllType (quantifiers, tp) ->
        ...
        ForAllType(
            ...
            tp = applySubstToType (substWithoutBound, tp)
        )
...
let rec contains (name: string, t: Type) =
    ...
    | VarType n -> n = name
    | FunctionType (param, body) ->
        contains (name, param) || contains (name, body)
...
let rec unify (t1: Type, t2: Type) =
    ...
    | (FunctionType (p1, b1), FunctionType (p2, b2)) ->
        ...
        let s2 =
            unify (applySubstToType (s1, b1), applySubstToType (s1, b2))
...
let tFunc (types) =
    types
    |> Array.reduceBack (fun p b -> FunctionType(p, b))
...
//
// Currying(https://en.wikipedia.org/wiki/Currying) 方式表示多参函数
// f(x, y) => f(x)(y)
let call (args) =
    args
    |> Array.reduce (fun f a -> CallExp(func = f, arg = a))
...
let rec freeTypeVarsInType (t: Type) =
    ...
    | FunctionType (param, body) ->
        union (freeTypeVarsInType (param), freeTypeVarsInType (body))
    | ForAllType (quantifiers, tp) ->
...
let generalize (env: Map<string, Type>, t: Type) =
    ...
    let quantifiers =
        difference (typeFreeVars, envFreeVars)
        |> Map.toArray
        |> Array.map fst
...
//        | None -> raise(InferException("Unbound var" + name))
    ...
    | FunctionExp (param, body) ->
        ...
        (FunctionType(param = applySubstToType (subst, nT), body = tBody), subst)
    ...
    | CallExp (func, arg) ->
        ...
        let s4 =
            unify (funcType, FunctionType(argType, nVar))

        let resultSubst = composeSubst (s3, s4)

        //
        let body = applySubstToType (resultSubst, nVar)
        (body, resultSubst)
    //        // OR
...
//        | t -> raise (InferException(typeToString (t)))
    ...
    | IfExp (cond, trueBranch, falseBranch) ->
        ...
        let s6 =
            unify (trueBranchType1, falseBranchType1)
    ...
    try
        let c =
            LetExp(
                name = "id",
                rhs = FunctionExp(param = "x", body = VarExp("x")),
                body = CallExp(func = VarExp("id"), arg = VarExp("true"))
            )
